...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isButton()) {
    if (interaction.customId.startsWith("claim_job_")) {
      ...
      const routeMatch = description.match(
        /\*\*🛫 Route:\*\* (.+?) ✈️ (.+?)\n/,
      );
      ...
      const disabledClaimButton = Discord.ButtonBuilder.from(
    ...
    if (interaction.customId.startsWith("mark_done_")) {
      ...
      const claimedByField = fields.find(
      ...
      if (claimedByField && claimedByField.value !== "None") {
        ...
        if (userIdMatch) {
          ...
          if (userData[userId] && userData[userId].activeJobs) {
            userData[userId].activeJobs = userData[userId].activeJobs.filter(
              (job) => job.jobId !== jobId,
            );
            saveUserData(userData);
          }
      ...
      const updatedEmbed = Discord.EmbedBuilder.from(originalEmbed)
        .setFields(updatedFields)
        .setColor("#32CD32"); // Change color to green when done
      ...
      const disabledDoneButton = Discord.ButtonBuilder.from(
...
// Upon client ready, set status and register commands, then log in
client.once("ready", () => {
